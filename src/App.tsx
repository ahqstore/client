...
import { PersonAddRegular, PersonRegular, ToolboxRegular, BugArrowCounterclockwiseRegular } from "@fluentui/react-icons"
...
function App() {
  ...
  return (
    <>
      {platform() != "android" && <WindowTitlebar
          ...
          <>
            <Popover>
              <PopoverTrigger className="border-none mx-3 cursor-pointer font-mono flex text-center items-center justify-center transition-all hover:scale-125 active:scale-90">
                {auth?.avatar_url ? <img src={auth?.avatar_url} className="mr-1 my-1 size-6 avatar rounded-xl" />
                  :
                  <PersonRegular className="mr-1 my-1 p-1 size-6 avatar rounded-full border border-neutral-content" />
                }
              </PopoverTrigger>

              <PopoverContent className={`!bg-accent shadow-sm w-auto h-auto p-2 backdrop-blur-3xl stylish rounded-lg border border-neutral-content/20`}>
                <MenuList className="bg-transparent">
                  ...
                  >
                    <div className="flex">
                      {auth?.avatar_url
                        ? <img src={auth?.avatar_url} className="h-12 avatar rounded-xl" />
                        : <PersonRegular className="size-12 p-2 border border-neutral-content rounded-full" />
                      }

                      <div className="ml-3 flex flex-col">
                        {auth ?
                          <span>Hello, <strong>{auth.name || auth.login}</strong></span>
                          :
                  ...
                  {auth &&
                    <MenuItem
                      icon={<ToolboxRegular />}
                      subText={auth.dev ? "Enabled" : "Register for developer mode"}
                      style={{ background: "transparent" }}
                      onClick={() => open("https://github.com/ahqstore/repo_community")}
                    >
                      Developer Mode
                    </MenuItem>
                  }
                  <MenuItem
                    icon={auth == undefined ? <PersonAddRegular /> : <LogOutIcon />}
                    subText={auth == undefined ? "Login using GitHub authentication" : "Logout from this device"}
                    onClick={auth == undefined ? () => startLogin(authObject) : () => logOut(authObject)}
                    className="w-72"
                    style={{ background: "transparent" }}
                  >
