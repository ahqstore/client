...
pub(crate) async fn get_all_search(app: AppHandle, query: &str) -> Result<Vec<RespSearchEntry>> {
  Ok(search::get_search(Some(&*app.ahqstore().commits.lock().await), query).await?)
}
...
pub(crate) async fn get_app(appl: AppHandle, app: &str) -> Result<AHQStoreApplication> {
  Ok(internet::get_app(&*appl.ahqstore().commits.lock().await, app).await?)
}
...
pub(crate) async fn get_dev_data(app: AppHandle, dev: &str) -> Result<DevData> {
  Ok(internet::get_dev_data(&*app.ahqstore().commits.lock().await, dev).await?)
}

#[command(async)]
pub(crate) async fn get_devs_apps(app: AppHandle, dev: &str) -> Result<Vec<String>> {
  Ok(internet::get_devs_apps(&*app.ahqstore().commits.lock().await, dev).await?)
}
